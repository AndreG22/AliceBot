 /*
* AliceBot es una creación de Andre
* AliceBot no tiene ningun fin de lucro
...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Alice\n' // Nombre
            + 'ORG:Andre;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=50687471084:+506 8747 1084\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******FIN DE ARCHIVOS ANTILINK POR ANDRE******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Andre'; if (!author) author = 'Alice';	
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa AliceBot',
					...
					daftarB: `「NO ESTAS REGISTRADO」\n\nPA NO APARECES EN MI BASE DE DATOS ✋🥸🤚\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar Andre`,
				}
			}
    			const apakah = ['Si','No']
                        const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
			const botNumber = client.user.jid
			const ownerNumber = ["50687471084@s.whatsapp.net"] // replace this with your number
...
/******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'AliceBot'; if (!author) author = 'Andre';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
...
/******JUEGOS ALICE LA PUTA MADRE NO TE OLVIDES******/
...
/******JUEGOS ALICE LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
		...
		client.sendMessage(from, 'Hola 👋 te saluda Alice.',MessageType.text, { quoted: mek} )
                const none = fs.readFileSync('./mp3/alice.mp3');
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*by Andre*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
			...
			//ANTILINKS DE REDES SOCIALES FLACO ACEPTALO SOLO LO ESTAS EDITANDO REALMENTE SHANDUY TE HIZO TODO ESTO	
				...
				//FIN DE ANTILINK HECHO POR Andre
...
//NSFW FIN DE FUNCIONES					
		...
		if (budy.includes(`Se imagina`)) {
                  reply(`Se envagina`)
                  }
	
	if (budy.startsWith(`Andres`)) {
        const none = fs.readFileSync('./mp3/andres.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }			
	if (budy.startsWith(`gracias`)) {
        const none = fs.readFileSync('./mp3/gracias.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
	...
	if (budy.startsWith(`pene`)) {
        const none = fs.readFileSync('./mp3/pene.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
        if (budy.startsWith(`tetas`)) {
        const none = fs.readFileSync('./mp3/tetas.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
		if (budy.startsWith(`culo`)) {
        const none = fs.readFileSync('./mp3/tetas.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
